...
void drawUserButtons()
{
	...
	if (button(GenUIID(0), x, y -= 1.5 * h, w, h, "查看上一个产品"))
	{
	...
	if (button(GenUIID(0), x + 1.2 * w, y += h * 1.5, w, h, "查看用户帮助"))
	{
		system("UserHelp.txt"); 
	}
	...
	if (button(GenUIID(0), x, y -= h * 1.5, w, h, "进入预订界面")) 
	{
		status = 5; 
		Returnflag = 0;
	}
	if (button(GenUIID(0), x + 1.2 * w, y, w, h, "反馈"))
	{
		status = 10; 
	}
}
...
{   
	...
	fH = GetFontHeight();  //设置绘制参数 
	h = fH * 2;  
	x = winwidth / 2.5;
	y = winheight / 1.5;
	w = winwidth / 3; 
	
	drawProduct(fH, x, y + h, w, h, Temp_Product); //绘制产品信息 
	
	SetPointSize(25);  //绘制产品标题 
	SetPenColor("Violet");
	drawLabel(winwidth / 2.2, y + 2 * h, "产品概述："); 
	SetPenColor("Green");
	SetPointSize(fH);
}
